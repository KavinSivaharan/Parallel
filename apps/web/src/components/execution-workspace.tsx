...
export function ExecutionWorkspace() {
  ...
  const refreshBranch = useCallback(
    async (branchId: string, after = 0) => {
      const [nextState, nextEvents, nextArtifacts, nextCollaborators] = await Promise.all([
      ...
      setState(nextState);
      setEvents((current) => {
        const base = after === 0 ? [] : current;
        return orderedUnique([...base, ...nextEvents]);
      });
    ...
    socket.on("event.committed", (event: EventEnvelope) => {
      setEvents((current) => {
        ...
        if (event.sequence !== expected) {
          void refreshBranch(session.branchId, current.at(-1)?.sequence ?? 0);
          return current;
        }
      ...
      void Promise.all([
        ...
        request<Artifact[]>(`/v1/branches/${session.branchId}/artifacts`).then(setArtifacts),
  ...
  }, [auth, refreshBranch, request, session]);
  ...
  async function command(type: string, payload: Record<string, unknown>): Promise<void> {
    ...
    try {
      ...
      await refreshBranch(session.branchId, lastSequence);
    } catch (reason) {
    ...
    <main className="shell">
      <header className="topbar">
        ...
        <div className="session-title">
          ...
          <button className="branch"><GitBranch size={13} /> main</button>
        </div>
          ...
          <section>
            ...
            <div className="artifact-list">
              {artifacts.map((artifact) => (
                <div key={artifact.id}><Code2 size={14} /><span><strong>{artifact.name}</strong><small>{artifact.mediaType} · {artifact.byteSize} B</small></span></div>
              ))}
...
function SessionPicker({
  ...
  const [title, setTitle] = useState("Build resilient token rotation");
  async function create(event: React.FormEvent) {
    event.preventDefault();
    const created = await request<{ sessionId: string; branchId: string }>("/v1/sessions", {
      method: "POST",
      body: JSON.stringify({ organizationId: organization.id, title, providerId: "simulator" }),
    });
    onCreated({ ...created, title, providerId: "simulator", createdAt: new Date().toISOString() });
  }
  return (
    <main className="session-home">
      ...
      <div className="session-home-body">
        ...
        <section>
          ...
          <form className="new-session" onSubmit={create}><input value={title} onChange={(event) => setTitle(event.target.value)} /><button><Plus size={15} /> Start simulator session</button></form>
...
function Composer({ isDriver, disabled, onSubmit, onComment }: {
  isDriver: boolean;
  disabled: boolean;
  ...
  const [mode, setMode] = useState<"steer" | "comment">("steer");
  async function submit(event: React.FormEvent) {
    event.preventDefault();
  ...
  return (
    <form className="composer" onSubmit={submit}>
      <div className="composer-modes"><button type="button" className={mode === "steer" ? "active" : ""} onClick={() => setMode("steer")}><Send size={12} /> {isDriver ? "Instruction" : "Steering proposal"}</button><button type="button" className={mode === "comment" ? "active" : ""} onClick={() => setMode("comment")}><MessageSquare size={12} /> Comment</button></div>
  ...
  return (
    <article className={`event event-${presentation.kind}`}>
      ...
      <div className="event-line">
        <span className="event-icon">{presentation.kind === "provider" ? <Code2 size={14} /> : presentation.kind === "comment" ? <MessageSquare size={14} /> : presentation.kind === "steering" ? <Send size={14} /> : presentation.kind === "tool" ? <TerminalSquare size={14} /> : <CircleDot size={14} />}</span>
